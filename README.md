...
Routine tests skip seven desktop-interactive checks that visibly open, close and focus temporary notes. Read `RULES.md` before testing. Agents must not set `SHIORI_RUN_WINDOW_TESTS=1` unless the user explicitly requests desktop-interactive tests for the current task; a request to build, test or relaunch does not authorize them.

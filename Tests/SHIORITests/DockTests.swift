...
final class DockTests: XCTestCase {
    ...
    func testRestingStripesDragAndReturnAfterHover() async throws {
        ...
        let actions = try XCTUnwrap(view.accessibilityChildren() as? [NSAccessibilityElement])
        let createAction = try XCTUnwrap(actions.first { $0.accessibilityLabel() == "Create note" })
        let moveAction = try XCTUnwrap(actions.first { $0.accessibilityLabel() == "Move edge tabs" })
        let createFrame = panel.convertFromScreen(createAction.accessibilityFrame())
        let moveFrame = panel.convertFromScreen(moveAction.accessibilityFrame())
        XCTAssertGreaterThan(createFrame.minY, moveFrame.maxY)
        let grip = NSPoint(x: moveFrame.midX, y: moveFrame.midY)
        XCTAssertNotNil(view.hitTest(grip))
        let anchorBeforeGrip = settings.anchor
        view.mouseDown(with: event(.leftMouseDown, grip))
        view.mouseDragged(with: event(.leftMouseDragged, NSPoint(x: grip.x, y: grip.y + 20)))
        view.mouseUp(with: event(.leftMouseUp, grip))
        XCTAssertGreaterThan(settings.anchor, anchorBeforeGrip)
        let first = try XCTUnwrap(store.active.first?.id)
        view.mouseDown(with: event(.leftMouseDown, NSPoint(x: 245, y: 381)))
        view.mouseDragged(with: event(.leftMouseDragged, NSPoint(x: 245, y: 146)))
        view.mouseUp(with: event(.leftMouseUp, NSPoint(x: 245, y: 146)))
        for _ in 0..<50 {
